...
export default function LandingPage() {
  ...
  return (
    <Grid container>
      <Grid item xs={1}></Grid>
      <Grid item xs={10}>
        <Typography sx={{ mb: '2rem', mt: '1rem' }} textAlign='center' variant='h4'>
          Welcome to Your Institution&apos;s Lab Training Tracker!
        </Typography>
        ...
        <Box display='flex' justifyContent='space-between' sx={{ flexDirection: { xs: 'column', sm: 'row' } }}>
          <Box display='flex' sx={{ mb: '1rem' }}>
            ...
            />
            <Button
              sx={{ backgroundColor: 'gray', ':hover': { backgroundColor: '#696969' }, marginLeft: '.75em' }}
              variant='contained'
          ...
          <Box sx={{ mb: '1rem' }}>
            <Button sx={{ mb: '.1rem' }} variant='contained' onClick={() => setDialogIsOpen(true)}>
              Add a New Person
            </Button>
          </Box>
        </Box>
            ...
            <TableBody>
              ...
              {peopleToDisplay?.map((person) => (
                ...
                <TableRow
                  ...
                  onClick={(e) => navigate(`/${e.currentTarget.dataset.person_id}/details`)}>
                  <TableCell>{person.last_name}</TableCell>
                  <TableCell>{person.first_name}</TableCell>
                  <TableCell>{person.email}</TableCell>
                  <TableCell>{person.is_instructor ? 'Yes' : 'No'}</TableCell>
                  <TableCell>
                    <Button data-person_id={person.id} color='error' variant='outlined' onClick={removePerson}>
                      Remove
                    </Button>
                  </TableCell>
                </TableRow>
